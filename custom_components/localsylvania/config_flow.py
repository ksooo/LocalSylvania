...
CLOUD_SETUP_SCHEMA = vol.Schema(
    {
        vol.Required(CONF_CLOUD_TYPE, default=CLOUD_TYPE_OEM_SYLVANIA): vol.In({
            CLOUD_TYPE_OEM_SYLVANIA: "Sylvania (email/password)",
            CLOUD_TYPE_OEM_LEDVANCE: "Ledvance (email/password)",
            CLOUD_TYPE_IOT: "Tuya IoT Platform (client_id/secret/user_id)",
            CLOUD_TYPE_OEM_GENERIC: "Generic OEM (email/password + client_id/secret)",
        }),
        vol.Required(CONF_REGION, default="us"): vol.In(["eu", "us", "cn", "in"]),
        vol.Optional(CONF_OEM_EMAIL): cv.string,
        vol.Optional(CONF_OEM_PASSWORD): cv.string,
...
async def attempt_cloud_connection(hass, user_input):
    """Create device."""
    # Use the factory function to create the appropriate cloud API
